...
The recipient maintains a control reserve of two groups. SURB groups do not have separate reserved and unreserved representations. Data and ACK traffic may consume a group only when at least two groups will remain afterward. When supply reaches the low-water mark and no refill is pending, the recipient consumes one group from that protected minimum to send `RefillRequest`.

One request asks for at most two groups, the maximum currently accepted in one returned Sphinx packet. The initiator creates those public SURBs, registers their private credentials, and sends one forward `Refill`. Locally encoded frames require every SURB group to be well formed. On receipt, structural frame validation is separated from per-group SURB decoding so that one malformed group does not reject valid groups in the same response. The recipient processes only a `Refill` whose batch ID matches its pending request, decodes every group independently and retains every valid group. The matching response completes the batch even when some groups are malformed. If the retained supply is still no greater than the control reserve, the blocked return send starts another one-packet refill batch. This allows a valid subset to preserve the session's ability to replenish itself instead of discarding useful SURBs with the malformed groups.

A matching response with too few valid groups causes an immediate subsequent refill request. A submitted refill currently has no retry timer, so recipient-originated traffic can still remain blocked if every redundant request copy or the complete response is lost. Timeout-driven refill retry remains required reliability work.
